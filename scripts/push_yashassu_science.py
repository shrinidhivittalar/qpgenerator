"""
Upsert Yashassu Science questions into MongoDB.
...
SOURCE_FILE = ROOT / "parsed_output_yashassu_science" / "questions.json"
if not SOURCE_FILE.exists():
    sys.exit(f"ERROR: {SOURCE_FILE} not found — run parse_mineru_with_gemini.py first")

with open(SOURCE_FILE, encoding="utf-8") as f:
    questions = list(json.load(f).values())

# Tag fields the parser doesn't set but mongo schema expects
for q in questions:
    q.setdefault("subject", "science")
    q.setdefault("source",  "yashassu_science")
